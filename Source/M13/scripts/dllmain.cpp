					...
					)
{
	...
	switch (ul_reason_for_call)
	{
		case DLL_PROCESS_ATTACH:
			...
			if (Exe != 1)
			{
				MessageBox(HWND_DESKTOP, "Fatal error: SSGM only works on the FDS. Please replace your scripts.dll with an appropriate version for the Renegade client.", "Renegade", MB_OK | MB_ICONERROR);
				exit(0);
			}
#endif
...
bool SCRIPTSAPI Set_Script_Commands(ScriptCommandsClass* commands)
{
	...
	{
		Console_Output("Fatal Error: scripts.dll requires a bandtest.dll version of %f. You are running %f.",TT_VERSION,GetTTVersion());
		return false;
	}
