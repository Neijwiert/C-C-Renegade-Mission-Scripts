...
#include "General.h"
#include "M01_CHURCH_Chinook_Spawned_Soldier02_GDI.h"
...
void M01_CHURCH_Chinook_Spawned_Soldier02_GDI::Custom(GameObject *obj, int type, int param, GameObject *sender)
{
	...
	if (param == 67)
	{
		if (this->churchAreaSecure)
		{
			...
			{
				homeLocation = Vector3(-139.048f, 318.86099f, 9.066f);
			}
			else if (rand >= 1.5f && rand < 2.5f)
			{
				homeLocation = Vector3(-81.160004f, 333.117f, 10.131f);
			}
			...
			{
				homeLocation = Vector3(-140.785f, 348.38901f, 21.931f);
			}
			else if (rand >= 4.5f && rand < 5.5f)
			{
				homeLocation = Vector3(-140.79201f, 342.98001f, 10.387f);
			}
			else
			{
				homeLocation = Vector3(-126.169f, 335.22299f, 8.9969997f);
			}
	...
	else if (param == 149)
	{
		if (!this->churchAreaSecure)
		{
			...
			if (rand >= 0.5f && rand < 1.5f)
			{
				homeLocation = Vector3(-160.217f, 446.353f, 34.976002f);
			}
			else if (rand >= 1.5f && rand < 2.5f)
			{
			...
			{
				homeLocation = Vector3(-138.95799f, 435.69601f, 27.000999f);
			}
	...
	else if (param == 150)
	{
		if (!this->churchAreaSecure)
		{
			...
			if (rand >= 0.5f && rand < 1.5f)
			{
				homeLocation = Vector3(-36.192001f, 319.67099f, 5.2259998f);
			}
			else if (rand >= 1.5f && rand < 2.5f)
			{
				homeLocation = Vector3(-49.613998f, 363.30399f, 10.816f);
			}
			else
			{
				homeLocation = Vector3(-66.361f, 339.28799f, 8.7720003f);
			}
...
void M01_CHURCH_Chinook_Spawned_Soldier02_GDI::Animation_Complete(GameObject *obj, const char *animation_name)
{
...
void M01_CHURCH_Chinook_Spawned_Soldier02_GDI::Poked(GameObject *obj, GameObject *poker)
{
	Vector3 pos = Commands->Get_Position(obj);
...
ScriptRegistrant<M01_CHURCH_Chinook_Spawned_Soldier02_GDI> M01_CHURCH_Chinook_Spawned_Soldier02_GDIRegistrant("M01_CHURCH_Chinook_Spawned_Soldier02_GDI", "");