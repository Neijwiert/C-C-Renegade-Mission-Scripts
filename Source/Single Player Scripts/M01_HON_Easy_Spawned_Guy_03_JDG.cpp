...
#include "General.h"
#include "M01_HON_Easy_Spawned_Guy_03_JDG.h"

void M01_HON_Easy_Spawned_Guy_03_JDG::Register_Auto_Save_Variables()
{
...
void M01_HON_Easy_Spawned_Guy_03_JDG::Created(GameObject *obj)
{
	...
	params.Set_Movement(Vector3(-221.754f, 514.833f, 4.015f), 0.8f, 0.05f);

	Commands->Action_Goto(obj, params);
}

void M01_HON_Easy_Spawned_Guy_03_JDG::Killed(GameObject *obj, GameObject *killer)
{
	GameObject *M01MissionControllerObj = Commands->Find_Object(100376);

	if (!this->movingToStar)
	{
		Commands->Send_Custom_Event(obj, M01MissionControllerObj, 0, 210, 0.0f);
	}

	Commands->Send_Custom_Event(obj, M01MissionControllerObj, 0, 207, 0.0f);
}

void M01_HON_Easy_Spawned_Guy_03_JDG::Damaged(GameObject *obj, GameObject *damager, float amount)
{
...
void M01_HON_Easy_Spawned_Guy_03_JDG::Custom(GameObject *obj, int type, int param, GameObject *sender)
{
	// Received from ourselves after 30 seconds after this block
	// Received from M01_mission_Controller_JDG when param 206 is received
	if (param == 65)
	{
		if (obj)
		{
...
void M01_HON_Easy_Spawned_Guy_03_JDG::Sound_Heard(GameObject *obj, const CombatSound & sound)
{
	Vector3 pos = Commands->Get_Position(obj);
	GameObject *starObj = Commands->Get_A_Star(pos);
...
void M01_HON_Easy_Spawned_Guy_03_JDG::Enemy_Seen(GameObject *obj, GameObject *enemy)
{
	Vector3 pos = Commands->Get_Position(obj);
...
void M01_HON_Easy_Spawned_Guy_03_JDG::Action_Complete(GameObject *obj, int action_id, ActionCompleteReason complete_reason)
{
	...
	// Triggered when we finished movement at create
	else if (action_id == 38 && complete_reason == ACTION_COMPLETE_NORMAL)
	{
		GameObject *M01MissionControllerObj = Commands->Find_Object(100376);
		Commands->Send_Custom_Event(obj, M01MissionControllerObj, 0, 206, 0.0f);
	}
}

ScriptRegistrant<M01_HON_Easy_Spawned_Guy_03_JDG> M01_HON_Easy_Spawned_Guy_03_JDGRegistrant("M01_HON_Easy_Spawned_Guy_03_JDG", "");